...
$layout->blocks["top"][] = "details";$page_layouts["tutorial_edit"] = $layout;
...
$templatefile = "tutorial_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["tutorial_id"] = urldecode(postvalue("editid1"));
$savedKeys["tutorial_id"] = urldecode(postvalue("editid1"));
...
//	processing tutorial_name - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_tutorial_name = $pageObject->getControl("tutorial_name", $id);
		$control_tutorial_name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tutorial_name - end
//	processing tutorial_description - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_tutorial_description = $pageObject->getControl("tutorial_description", $id);
		$control_tutorial_description->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tutorial_description - end
//	processing tutorial_category - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_tutorial_category = $pageObject->getControl("tutorial_category", $id);
		$control_tutorial_category->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tutorial_category - end
	...
	//	do event
		...
		{		
			...
			//	processing tutorial_name - begin
				...
				if($condition)
				{
					$control_tutorial_name->afterSuccessfulSave();
				}
	//	processing tutorial_name - end
			//	processing tutorial_description - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_tutorial_description->afterSuccessfulSave();
				}
	//	processing tutorial_description - end
			//	processing tutorial_category - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_tutorial_category->afterSuccessfulSave();
				}
	//	processing tutorial_category - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["tutorial_id"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: tutorial_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: tutorial_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["tutorial_name"] = $evalues["tutorial_name"];
	$data["tutorial_description"] = $evalues["tutorial_description"];
	$data["tutorial_category"] = $evalues["tutorial_category"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("tutorial_name"))
		$xt->assign("tutorial_name_fieldblock",true);
	else
		$xt->assign("tutorial_name_tabfieldblock",true);
	$xt->assign("tutorial_name_label",true);
	if(isEnableSection508())
		$xt->assign_section("tutorial_name_label","<label for=\"".GetInputElementId("tutorial_name", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("tutorial_description"))
		$xt->assign("tutorial_description_fieldblock",true);
	else
		$xt->assign("tutorial_description_tabfieldblock",true);
	$xt->assign("tutorial_description_label",true);
	if(isEnableSection508())
		$xt->assign_section("tutorial_description_label","<label for=\"".GetInputElementId("tutorial_description", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("tutorial_category"))
		$xt->assign("tutorial_category_fieldblock",true);
	else
		$xt->assign("tutorial_category_tabfieldblock",true);
	$xt->assign("tutorial_category_label",true);
	if(isEnableSection508())
		$xt->assign_section("tutorial_category_label","<label for=\"".GetInputElementId("tutorial_category", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("tutorial_id", $data)));
...
//	detail tables
	$showDetailKeys["steps"]["masterkey1"] = $data["tutorial_id"];		

	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["tutorial_id"]));
...
//	tutorial_name - 
	$value = $pageObject->showDBValue("tutorial_name", $data, $keylink);
	$showValues["tutorial_name"] = $value;
	$showFields[] = "tutorial_name";
		$showRawValues["tutorial_name"] = substr($data["tutorial_name"],0,100);

//	tutorial_description - HTML
	$value = $pageObject->showDBValue("tutorial_description", $data, $keylink);
	$showValues["tutorial_description"] = $value;
	$showFields[] = "tutorial_description";
		$showRawValues["tutorial_description"] = substr($data["tutorial_description"],0,100);

//	tutorial_category - 
	$value = $pageObject->showDBValue("tutorial_category", $data, $keylink);
	$showValues["tutorial_category"] = $value;
	$showFields[] = "tutorial_category";
		$showRawValues["tutorial_category"] = substr($data["tutorial_category"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='tutorial_view.php?".$viewlink."'\"");
