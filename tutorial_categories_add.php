...
$layout->blocks["top"][] = "details";$page_layouts["tutorial_categories_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tutorial_categories_inline_add.htm";
else
	$templatefile = "tutorial_categories_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tutorial_categories_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing cat_name - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_cat_name = $pageObject->getControl("cat_name", $id);
		$control_cat_name->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing cat_name - end
//	processing cat_description - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_cat_description = $pageObject->getControl("cat_description", $id);
		$control_cat_description->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tutorial_categories_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tutorial_categories_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tutorial_categories_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["cat_id"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["cat_id"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["cat_name"]=@$avalues["cat_name"];
	$defvalues["cat_description"]=@$avalues["cat_description"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("cat_name_label","<label for=\"".GetInputElementId("cat_name", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("cat_description_label","<label for=\"".GetInputElementId("cat_description", $id, PAGE_ADD)."\">","</label>");
