...
$layout->blocks["top"][] = "details";$page_layouts["tutorial_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "tutorial_inline_add.htm";
else
	$templatefile = "tutorial_add.htm";
...
if(isset($_REQUEST['afteradd'])){
		header('Location: tutorial_add.php');
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing tutorial_name - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_tutorial_name = $pageObject->getControl("tutorial_name", $id);
		$control_tutorial_name->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing tutorial_name - end
//	processing tutorial_description - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_tutorial_description = $pageObject->getControl("tutorial_description", $id);
		$control_tutorial_description->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing tutorial_description - end
//	processing tutorial_category - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_tutorial_category = $pageObject->getControl("tutorial_category", $id);
		$control_tutorial_category->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing tutorial_category - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="tutorial_categories")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["tutorial_category"]==""){
			$avalues["tutorial_category"] = prepare_for_db("tutorial_category",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'tutorial_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'tutorial_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: tutorial_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["tutorial_id"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["tutorial_id"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="tutorial_categories")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["tutorial_category"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["tutorial_name"]=@$avalues["tutorial_name"];
	$defvalues["tutorial_description"]=@$avalues["tutorial_description"];
	$defvalues["tutorial_category"]=@$avalues["tutorial_category"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("tutorial_name_label","<label for=\"".GetInputElementId("tutorial_name", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("tutorial_description_label","<label for=\"".GetInputElementId("tutorial_description", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("tutorial_category_label","<label for=\"".GetInputElementId("tutorial_category", $id, PAGE_ADD)."\">","</label>");
