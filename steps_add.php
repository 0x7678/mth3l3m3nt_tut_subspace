...
$layout = new TLayout("add2","Rounded1Blue_white1","MobileBlue_white1");
...
$layout->blocks["top"][] = "details";$page_layouts["steps_add"] = $layout;
...
if($inlineadd == ADD_INLINE)
	$templatefile = "steps_inline_add.htm";
else
	$templatefile = "steps_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing tutorial_id - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_tutorial_id = $pageObject->getControl("tutorial_id", $id);
		$control_tutorial_id->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing tutorial_id - end
//	processing step_title - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_step_title = $pageObject->getControl("step_title", $id);
		$control_step_title->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing step_title - end
//	processing step_description - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_step_description = $pageObject->getControl("step_description", $id);
		$control_step_description->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing step_description - end
//	processing step_screenshot - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
		$control_step_screenshot = $pageObject->getControl("step_screenshot", $id);
		$control_step_screenshot->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing step_screenshot - end


//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="tutorial")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["tutorial_id"]==""){
			$avalues["tutorial_id"] = prepare_for_db("tutorial_id",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if($pageObject->pSet->hasEditPage() && $permis['edit'])
						$message .='&nbsp;<a href=\'steps_edit.php?'.$keylink.'\'>'."Edit".'</a>&nbsp;';
					if($pageObject->pSet->hasViewPage() && $permis['search'])
						$message .='&nbsp;<a href=\'steps_view.php?'.$keylink.'\'>'."View".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: steps_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["step_id"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["step_id"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="tutorial")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["tutorial_id"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
	
}

if($readavalues)
{
	$defvalues["tutorial_id"]=@$avalues["tutorial_id"];
	$defvalues["step_title"]=@$avalues["step_title"];
	$defvalues["step_description"]=@$avalues["step_description"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("tutorial_id_label","<label for=\"".GetInputElementId("tutorial_id", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("step_title_label","<label for=\"".GetInputElementId("step_title", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("step_description_label","<label for=\"".GetInputElementId("step_description", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("step_screenshot_label","<label for=\"".GetInputElementId("step_screenshot", $id, PAGE_ADD)."\">","</label>");
