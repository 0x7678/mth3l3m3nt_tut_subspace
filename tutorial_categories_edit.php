...
$layout->blocks["top"][] = "details";$page_layouts["tutorial_categories_edit"] = $layout;
...
$templatefile = "tutorial_categories_edit.htm";
...
$skeys = "";
$savedKeys = array();
$keys["cat_id"] = urldecode(postvalue("editid1"));
$savedKeys["cat_id"] = urldecode(postvalue("editid1"));
...
//	processing cat_name - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_cat_name = $pageObject->getControl("cat_name", $id);
		$control_cat_name->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing cat_name - end
//	processing cat_description - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_cat_description = $pageObject->getControl("cat_description", $id);
		$control_cat_description->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing cat_description - end
	...
	//	do event
		...
		{		
			...
			//	processing cat_name - begin
				...
				if($condition)
				{
					$control_cat_name->afterSuccessfulSave();
				}
	//	processing cat_name - end
			//	processing cat_description - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_cat_description->afterSuccessfulSave();
				}
	//	processing cat_description - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["cat_id"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: tutorial_categories_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: tutorial_categories_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["cat_name"] = $evalues["cat_name"];
	$data["cat_description"] = $evalues["cat_description"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("cat_name"))
		$xt->assign("cat_name_fieldblock",true);
	else
		$xt->assign("cat_name_tabfieldblock",true);
	$xt->assign("cat_name_label",true);
	if(isEnableSection508())
		$xt->assign_section("cat_name_label","<label for=\"".GetInputElementId("cat_name", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("cat_description"))
		$xt->assign("cat_description_fieldblock",true);
	else
		$xt->assign("cat_description_tabfieldblock",true);
	$xt->assign("cat_description_label",true);
	if(isEnableSection508())
		$xt->assign_section("cat_description_label","<label for=\"".GetInputElementId("cat_description", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("cat_id", $data)));
...
//	detail tables
	$showDetailKeys["tutorial"]["masterkey1"] = $data["cat_id"];		
	$showDetailKeys["exploit_store"]["masterkey1"] = $data["cat_id"];		
...
//	cat_id - 
	$value = $pageObject->showDBValue("cat_id", $data, $keylink);
	$showValues["cat_id"] = $value;
	$showFields[] = "cat_id";
		$showRawValues["cat_id"] = substr($data["cat_id"],0,100);

//	cat_name - 
	$value = $pageObject->showDBValue("cat_name", $data, $keylink);
	$showValues["cat_name"] = $value;
	$showFields[] = "cat_name";
		$showRawValues["cat_name"] = substr($data["cat_name"],0,100);

//	cat_description - 
	$value = $pageObject->showDBValue("cat_description", $data, $keylink);
	$showValues["cat_description"] = $value;
	$showFields[] = "cat_description";
		$showRawValues["cat_description"] = substr($data["cat_description"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
if($pageObject->isShowDetailTables && $inlineedit!=EDIT_INLINE && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "tutorial_categories";
		$options['firstTime'] = 1;
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='tutorial_categories_view.php?".$viewlink."'\"");
