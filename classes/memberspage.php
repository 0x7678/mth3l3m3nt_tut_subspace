...
{	
	...
	*/
	...
	{	
		...
		//for display name 
		$displayuserheaderlink_attrs = "href=\"".$this->tName."_list.php?orderby="."Ausername\"";
		...
		if ($this->sortByGroup === false && $this->sortField != ""){
			if ($this->sortField == "username"){
				...
				if($this->sortOrder == "A") {
					...
					$displayuserheaderlink_attrs.="Dusername";
				} else {
					if($this->is508)
					...
					$displayuserheaderlink_attrs.="Ausername";
				}
	...
	function fillGridData() 
	{
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			$row["displayusername"] = htmlspecialchars($data["username"]); //htmlspecialchars($data["username"]);
	...
	function setSessionVariables() 
	{
		...
		if (substr($_SESSION[$this->sessionPrefix."_orderby"],1) == "username" || substr($_SESSION[$this->sessionPrefix."_orderby"],1) =="username"){
