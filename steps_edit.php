...
include("include/steps_variables.php");
...
$layout = new TLayout("edit2","Rounded1Blue_white1","MobileBlue_white1");
...
$layout->blocks["top"][] = "details";$page_layouts["steps_edit"] = $layout;
...
$skeys = "";
$savedKeys = array();
$keys["step_id"] = urldecode(postvalue("editid1"));
$savedKeys["step_id"] = urldecode(postvalue("editid1"));
...
//	processing tutorial_id - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_tutorial_id = $pageObject->getControl("tutorial_id", $id);
		$control_tutorial_id->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing tutorial_id - end
//	processing step_title - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_step_title = $pageObject->getControl("step_title", $id);
		$control_step_title->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing step_title - end
//	processing step_description - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_step_description = $pageObject->getControl("step_description", $id);
		$control_step_description->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing step_description - end
//	processing step_screenshot - begin
	$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode

	if($condition)
	{
		$control_step_screenshot = $pageObject->getControl("step_screenshot", $id);
		$control_step_screenshot->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);

		}
//	processing step_screenshot - end
	...
	//	do event
		...
		{		
			...
			//	processing tutorial_id - begin
				...
				if($condition)
				{
					$control_tutorial_id->afterSuccessfulSave();
				}
	//	processing tutorial_id - end
			//	processing step_title - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_step_title->afterSuccessfulSave();
				}
	//	processing step_title - end
			//	processing step_description - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_step_description->afterSuccessfulSave();
				}
	//	processing step_description - end
			//	processing step_screenshot - begin
							$condition = $inlineedit!=EDIT_INLINE;//(!$inlineedit) edit simple mode
				...
				if($condition)
				{
					$control_step_screenshot->afterSuccessfulSave();
				}
	//	processing step_screenshot - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["step_id"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: steps_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: steps_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["tutorial_id"] = $evalues["tutorial_id"];
	$data["step_title"] = $evalues["step_title"];
	$data["step_description"] = $evalues["step_description"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(!$pageObject->isAppearOnTabs("tutorial_id"))
		$xt->assign("tutorial_id_fieldblock",true);
	else
		$xt->assign("tutorial_id_tabfieldblock",true);
	$xt->assign("tutorial_id_label",true);
	if(isEnableSection508())
		$xt->assign_section("tutorial_id_label","<label for=\"".GetInputElementId("tutorial_id", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("step_title"))
		$xt->assign("step_title_fieldblock",true);
	else
		$xt->assign("step_title_tabfieldblock",true);
	$xt->assign("step_title_label",true);
	if(isEnableSection508())
		$xt->assign_section("step_title_label","<label for=\"".GetInputElementId("step_title", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("step_description"))
		$xt->assign("step_description_fieldblock",true);
	else
		$xt->assign("step_description_tabfieldblock",true);
	$xt->assign("step_description_label",true);
	if(isEnableSection508())
		$xt->assign_section("step_description_label","<label for=\"".GetInputElementId("step_description", $id, PAGE_EDIT)."\">","</label>");
		
	if(!$pageObject->isAppearOnTabs("step_screenshot"))
		$xt->assign("step_screenshot_fieldblock",true);
	else
		$xt->assign("step_screenshot_tabfieldblock",true);
	$xt->assign("step_screenshot_label",true);
	if(isEnableSection508())
		$xt->assign_section("step_screenshot_label","<label for=\"".GetInputElementId("step_screenshot", $id, PAGE_EDIT)."\">","</label>");
		

	$xt->assign("show_key1", htmlspecialchars($pageObject->showDBValue("step_id", $data)));
...
//	detail tables
	...
	$keylink = "";
	$keylink.= "&key1=".htmlspecialchars(rawurlencode(@$data["step_id"]));


//	step_id - 
	$value = $pageObject->showDBValue("step_id", $data, $keylink);
	$showValues["step_id"] = $value;
	$showFields[] = "step_id";
		$showRawValues["step_id"] = substr($data["step_id"],0,100);

//	tutorial_id - 
	$value = $pageObject->showDBValue("tutorial_id", $data, $keylink);
	$showValues["tutorial_id"] = $value;
	$showFields[] = "tutorial_id";
		$showRawValues["tutorial_id"] = substr($data["tutorial_id"],0,100);

//	step_title - 
	$value = $pageObject->showDBValue("step_title", $data, $keylink);
	$showValues["step_title"] = $value;
	$showFields[] = "step_title";
		$showRawValues["step_title"] = substr($data["step_title"],0,100);

//	step_description - HTML
	$value = $pageObject->showDBValue("step_description", $data, $keylink);
	$showValues["step_description"] = $value;
	$showFields[] = "step_description";
		$showRawValues["step_description"] = substr($data["step_description"],0,100);

//	step_screenshot - Database Image
	$value = $pageObject->showDBValue("step_screenshot", $data, $keylink);
	$showValues["step_screenshot"] = $value;
	$showFields[] = "step_screenshot";
		$showRawValues["step_screenshot"] = "";

//	thumb - Database Image
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton".$id."\" onclick=\"window.location.href='steps_view.php?".$viewlink."'\"");
