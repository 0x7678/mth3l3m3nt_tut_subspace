...
function checkTableName($shortTName, $type=false)
{
	...
	if ("steps" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tutorial" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("tutorial_categories" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("exploit_store" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("View_Tutorials" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	return false;
}
...
//	default permissions	
	if($table=="exploit_store")
	{
	return "ADESPI";// grant all by default
	}
//	default permissions	
	if($table=="View_Tutorials")
	{
	return "ADESPI";// grant all by default
	}
}
